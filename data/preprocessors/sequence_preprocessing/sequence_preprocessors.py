...
from data.preprocessors.sequence_preprocessing.processing import *
...
class AbstractSequencePreprocessor(AbstractPreprocessor, ABC):
    ...
    def split(self, split_value: float):
        ...
        first_pp = self.__class__(source_dataset=self.source_dataset, alphabet_size=self.alphabet_size,
                                       data_length=self.data_length)
        second_pp = self.__class__(source_dataset=self.source_dataset, alphabet_size=self.alphabet_size,
                                        data_length=self.data_length)
...
class NormalSequencePreprocessor(AbstractSequencePreprocessor):
    """Preprocessor returning sequence based tensors without padding. Special symbols will be inserted"""

    def __init__(self, source_dataset: AbstractDataset, alphabet_size: int, data_length=None,
                 feature_extractor=None, som=None, num_clusters=1):
    ...
    def __getitem__(self, idx):
        ...
        # output
        item_tensor = one_hot_encode_string(string=item[:-1])
        target_tensor = one_hot_encode_string(string=item[1:])
        return item_tensor, target_tensor


class PaddedNormalSequencePreprocessor(NormalSequencePreprocessor):
    """Preprocessor returning padded sequence based tensors. Special symbols will be inserted"""

    def __init__(self, source_dataset: AbstractDataset, alphabet_size: int, data_length: int,
                 feature_extractor=None, som=None, num_clusters=1):
        super().__init__(source_dataset=source_dataset, alphabet_size=alphabet_size, data_length=data_length)
        assert data_length > 0
        self.data_length = data_length
        self.aux_preprocessor = NormalImagePreprocessor(source_dataset=source_dataset, data_length=data_length)
        self.feature_extractor = feature_extractor
        self.som = som
        self.num_clusters = num_clusters
    ...
    def __getitem__(self, idx):
        ...
        while len(item) < 4 * self.data_length:
            try:
                ...
                # setting special symbols
                item += chr(self.alphabet_size - aux_data - 1) + data \
                       + chr(self.alphabet_size - self.num_clusters - aux_data - 1)
            except:
