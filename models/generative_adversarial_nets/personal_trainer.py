...
class GenerativeAdversarialNetPersonalTrainer(AbstractPersonalTrainer):
    ...
    """
    def __init__(self, generator, discriminator: torch.nn.Module, training_data,
                 test_data, log_interval, generator_save_path,
                 discrminator_save_path, criterion, g_optimizer, d_optimizer):

        super().__init__(generator, training_data, test_data, log_interval,
                         generator_save_path, criterion, g_optimizer)
    ...
    def train(self, epoch):
        ...
        def train_discriminator(real_data, target):
            ...
            returned
            """
            fake_data = self.generator(Variable(
                create_noise(real_data.shape).to(self.device))).detach()
            self.d_optimizer.zero_grad()
            prediction_fake = self.discriminator(fake_data)
            error_fake = self.criterion(prediction_fake, target=Variable(
                torch.zeros(real_data.shape[0], 1).to(self.device)))
            error_fake.backward()
        ...
        def train_generator(real_data, target):
            ...
            fake_data = self.generator(create_noise([real_data.shape[0], 1,
                                       real_data.shape[2]]).to(self.device))
        ...
        for batch_id, (real_data, target) in enumerate(self.training_data):
            ...
            if g_error < g_breaker or d_error_real >= d_breaker \
                    or d_error_fake >= d_breaker:
                d_error_real, d_error_fake = train_discriminator(real_data,
                                                                 target)

            # checking if generator needs training
            if d_error_real < d_breaker or d_error_fake < d_breaker \
                    or g_error >= g_breaker:
            ...
            if batch_id % self.log_interval == 0 and batch_id != 0:
                ...
                print("\n#### avg. discrimination error real: {:.4f}, avg. "
                      ...
                      "generation error: {:.4f} ####".format(d_error_real,
                                                             d_error_fake,
                                                             g_error))
    ...
    def test(self, epoch):
        ...
        for batch_id, (real_data, target) in enumerate(self.test_data):
            ...
            # getting loss
            fake_data = self.generator(create_noise(real_data.shape).
                                       to(self.device))
            prediction = self.discriminator(fake_data)
    ...
    def create_output(self, dims: []):
        with torch.no_grad():
            return self.generator.create_output(Variable(create_noise(dims).
