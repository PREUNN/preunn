...
from models.generative_adversarial_nets.helper import *
...
class GenerativeAdversarialNetPersonalTrainer(AbstractPersonalTrainer):
    ...
    def train(self, epoch):
        ...
        def train_discriminator(real_data, target):
            """
            separate training method for the discriminator, one data element at the time
            :param real_data: example of the real dataset
            :param target: target of the real data example
            :return: both the error for real data and fake data prediction is returned
            """
        ...
        def train_generator(real_data, target):
            """
            separate training method for the generator, one data element at the time
            :param real_data: example of the real dataset
            :param target: target of the real data example
        ...
        for batch_id, (real_data, target) in enumerate(self.training_data):
            ...
            if batch_id % self.log_interval == 0 and batch_id != 0:
                ...
                print("\n#### avg. discrimination error real: {:.4f}, avg. discrimination error fake: {:.4f} and avg. "
                      "generation error: {:.4f} ####".format(d_error_real, d_error_fake, g_error))

        # has to be saved here, since generator is automatically saved as the model
